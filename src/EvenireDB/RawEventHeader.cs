...
internal struct RawEventHeader
{
    ...
    public async Task SerializeAsync(Stream stream, CancellationToken cancellationToken = default)
    {
        ...
        var buffer = ArrayPool<byte>.Shared.Rent(HEADER_SIZE);
        ...
        // event data length
        Array.Copy(BitConverter.GetBytes(EventDataLength), 0, buffer, EVENTDATALENGTH_POS, sizeof(int));
        
        await stream.WriteAsync(buffer, 0, HEADER_SIZE, cancellationToken)
